...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		Iterator<BTreeEntry> iterator = p.iterator();
		BTreeEntry e = null;
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
		...
		Tuple leftmost = null;
		for (int i = 0; i < page.getMaxTuples()/2; i++) {
			if (it.hasNext()) {
				leftmost = it.next();
				page.deleteTuple(leftmost);
				newLeafPage.insertTuple(leftmost);
			}
		}

		// get parent internal page and copy leftmost up.
		BTreeEntry newEntry = new BTreeEntry(leftmost.getField(keyField), page.getId(), newLeafPage.getId());
		...
		// set pointers
		newLeafPage.setParentId(parent.getId());
		if (page.getRightSiblingId() != null) {
			...
			newLeafPage.setRightSiblingId(rightSibling.getId());
		}
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		for (int i = 0; i < page.getMaxEntries() / 2; i++) {
			if (it.hasNext()) {
				entry = it.next();
		...
		parent.insertEntry(new BTreeEntry(entry.getKey(), page.getId(), newInternalPage.getId()));

		// set pointers
		newInternalPage.setParentId(parent.getId());
		updateParentPointers(tid, dirtypages, newInternalPage);

		if (field.compare(Op.GREATER_THAN, entry.getKey())) {
	...
	protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		int moveCount = (sibling.getNumTuples() - page.getNumTuples()) / 2;
		...
		// new key in parent entry is always the lefmost key of the right page
		Field newKey = isRightSibling ? it.next().getField(keyField) : t.getField(keyField);
		entry.setKey(newKey);
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		int moveCount = (leftSibling.getNumEntries() - page.getNumEntries()) / 2;

		// find the left and right child for the new entry
		...
		// move entries from left sibling to the page
		...
		// move the last key from left sibling to the parent internal page
		e = it.next();
		leftSibling.deleteKeyAndRightChild(e);
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		int moveCount = (rightSibling.getNumEntries() - page.getNumEntries()) / 2;
	...
	protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		if(it.hasNext()) {
			e = it.next();
			rightChildId = e.getLeftChild();
			leftPage.insertEntry(new BTreeEntry(parentEntry.getKey(), leftChildId, rightChildId));
